...
builder.Services.AddSingleton<StoryPitchValidator>();
builder.Services.AddSingleton<AssignmentAgentHandler>();
builder.Services.AddHostedService<AgentWorker<AssignmentAgentHandler>>();
...
app.MapGet("/", () => Results.Ok(new { service = "AssignmentAgent", status = "ready" }));
